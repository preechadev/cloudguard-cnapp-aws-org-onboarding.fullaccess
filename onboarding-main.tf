...
resource "dome9_cloudaccount_aws" "onboard-aws-account" {
  ...
  credentials  {
    arn    = "arn:aws:iam::${each.value.id}:role/CloudGuard-Connect-RW-role${var.cspm-aws-role-suffix}"
...
data "http" "github-chkp-repository" {
  url = "https://raw.githubusercontent.com/dome9/unified-onboarding/Release/cft/generated/templates/role_based/permissions_readwrite_cft.yml"
}
